...
script_wd <- getwd()
wd <- paste0(script_wd,"/genespace")
path2mcscanx <- paste0(script_wd,"/MCScanX")
...
plot_riparian(
  ...
  chrBorderCol = "black",
  addThemes = ggthemes,
  pdfFile = paste0(script_wd,"/figures/rip_hal_bp.pdf"))
plot_riparian(
  gsParam = gpar, 
  refGenome = "A.halleri",
  syntenyWeight = 0,
  ...
  chrBorderCol = "black",
  addThemes = ggthemes,
  pdfFile = paste0(script_wd,"/figures/rip_hal_genOrd.pdf"))


### Dotplot ###

# Use modified GENESPACE plotting functions:
source(paste0(script_wd,"/modified_dotplot.R"))
# Read in blast hits:
lyr_v_hal <- read_allBlast(paste0(wd,"/syntenicHits/A.halleri_vs_A.lyrata.allBlast.txt.gz"))
# Make plot:
ggdotplot(lyr_v_hal,type="syntenic",outDir = paste0(script_wd,"/figures/"))